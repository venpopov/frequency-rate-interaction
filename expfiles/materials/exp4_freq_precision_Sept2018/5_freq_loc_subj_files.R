...
all_words <- read.csv(file = "expfiles/materials/exp4_freq_precision_Sept2018/words.csv",stringsAsFactors = FALSE)


for (n in (1:number_of_subj_files))
{
  all_trials<-data.frame(matrix(ncol=26,nrow=310),stringsAsFactors = FALSE)
  colnames(all_trials)<-c('word1','word2','word3','word4','word5','angle1','angle2','angle3','angle4','angle5','target_word_place', 'target_word','target_angle','x1','y1','x2','y2','x3','y3','x4','y4','x5','y5','freq','delay','Procedure')
  ...
  for (m in (1:10))
  {
    ...
    colnames(HF_trials)<-c('word1','word2','word3','word4','word5','angle1','angle2','angle3','angle4','angle5','target_word_place', 'target_word','target_angle','x1','y1','x2','y2','x3','y3','x4','y4','x5','y5','freq','delay')
    ...
    HF_word_position_order_delay<-data.frame(matrix(nrow=15,ncol=2))
    
    HF_word_position_order_delay[,1]<-c(rep(1,3),rep(2,3),rep(3,3),rep(4,3),rep(5,3))
    HF_word_position_order_delay[,2]<-rep(c(150,250,350),5)
    HF_word_position_order_delay<-HF_word_position_order_delay[sample(nrow(HF_word_position_order_delay)),]
    
    HF_trials[,c(11,25)]<-HF_word_position_order_delay
    
    for (i in (1:15))
    {
      HF_trials[i,12] <- HF_trials[i,HF_word_position_order_delay[i,1]]
      HF_trials[i,13] <- HF_trials[i,HF_word_position_order_delay[i,1]+5]
    }
    ...
    colnames(LF_trials)<-c('word1','word2','word3','word4','word5','angle1','angle2','angle3','angle4','angle5','target_word_place', 'target_word','target_angle','x1','y1','x2','y2','x3','y3','x4','y4','x5','y5','freq','delay')
    ...
    ###### Generate order for target word position
    LF_word_position_order_delay<-data.frame(matrix(nrow=15,ncol=2))
    
    LF_word_position_order_delay[,1]<-c(rep(1,3),rep(2,3),rep(3,3),rep(4,3),rep(5,3))
    LF_word_position_order_delay[,2]<-rep(c(150,250,350),5)
    LF_word_position_order_delay<-LF_word_position_order_delay[sample(nrow(LF_word_position_order_delay)),]
    
    LF_trials[,c(11,25)]<-LF_word_position_order_delay
    
    for (i in (1:15))
    {
      LF_trials[i,12] <- LF_trials[i,LF_word_position_order_delay[i,1]]
      LF_trials[i,13] <- LF_trials[i,LF_word_position_order_delay[i,1]+5]
    }
  ...
  write.table(data, paste0("expfiles/materials/exp4_freq_precision_Sept2018/Subject_Files/loc_freq_subject_",toString(n),".txt"), sep="\t", quote= FALSE, row.names = FALSE)
